    ...
    try {
        ...
        if (coupons.length > 0) {
            ...
            if (req.query.id) {
                ...
                res.render('checkout', { cartProducts, subtotal:originalAmts , Address, user, couponCode, couponName,coupons:couponList});
            } else {
            ...
            if (req.query.id) {
                const userid = req.query.id;
                const Address = await address.findById({ _id: userid });
                res.render('checkout', { cartProducts, subtotal:originalAmts , Address, user, walletAmount ,coupons:couponList});
            } else {
                const Address = await address.findOne({ user: req.session.user._id });
                res.render('checkout', { cartProducts, subtotal:originalAmts , Address, user, walletAmount ,coupons:couponList});
            }
...
const LoadUserOrders = async (req, res) => {
    try {
        ...
        if (userOrder) {
            ...
            res.render('orderedProducts', { orderedProducts, userOrder ,user:req.session.user})
        } else {
...
const loadWalletPage = async (req, res) => {
    try {
        ...
        const findWallet = await user.findOne({ _id: req.session.user._id }).sort({ createdAt: -1 });

        
        res.render('wallet', { walletdata: findWallet, user:req.session.user });
    } catch (error) {
        console.log(error.message);
        res.status(500).render('error')
