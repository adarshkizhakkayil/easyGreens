...
const verifyOtp = async (req, res) => {
    try {
        if (req.session.otp === req.body.otp) {
            const data = await user.updateOne({ _id: userid }, { $set: { is_verified: 1 } }).exec();
...
const loadHome = async (req, res) => {
    try {
        if (req.session.user) {
            ...
            res.render('home', { userData, isLoggedIn: true ,user: req.session.user});
        } else {
    ...
    try {
        ...
        if (userData) {
            if (userData.is_verified === 0) {
                return res.render('login', { message: 'Please verify your email to log in.' });
            }
...
//         //     const limitedProducts = await products.find(query).skip(skip).limit(limit).populate('category');
//         //     totalPages = Math.ceil(count / limit);
//         //     return res.render('shop', { category: categoryCollection, product: limitedProducts, totalPages ,page,user:req.session.user});
...
const loadProductDetails = async (req, res) => {
    try {
        ...
        res.render('shop-details', { product: findproduct, Products1: relatedproducts ,category: categoryCollection});
    } catch (error) {
...
const resetPassword = async (req, res) => {
    try {
        ...
        const hashedPassword = await securepassword(newPassword);
        const updatePassword = await user.findOneAndUpdate({ email: usermail }, {
            $set: { password: hashedPassword }
        });
        if (updatePassword) {
            res.status(200).json({ message: 'password updated succesfully' });
